...
function ProjectCard({data}:{data:any}) {
  return (
    <Link href={data.titleLink} target='_blank' className='flex flex-col sm:flex-row  items-start group cursor-pointer glossy lg:p-5 rounded-lg drop-shadow-4xl justify-between gap-[20px]'>
        <div className='hidden  md:w-[25%] lg:w-[30%] sm:block'>
           <Image src={data.image} width={150} layout='responsive' className='rounded  border-[2px] border-gray-700'  alt='project'/>
        </div>
